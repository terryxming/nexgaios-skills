...
规则：一次 push 的提交范围内若动了 skills/** 或 docs/decisions/**，必须同批更新
根目录 handoff.md——防"改了正事、漏了交接"（纪律常驻 ≠ 执行器，见 lessons-learned）。
...
def main(base: str, head: str) -> int:
    ...
    if not watched:
        print("✅ handoff 联动检查：本次推送未动 skills/ 或 docs/decisions/")
        return 0
    if HANDOFF in files:
        print(f"✅ handoff 联动检查：{len(watched)} 个受关注变更，handoff.md 已同批更新")
        return 0
    print("✗ handoff 联动检查失败（C2 · ADR-0009）：动了 skills/ 或 docs/decisions/，但未更新 handoff.md")
    print("  受关注变更：\n    " + "\n    ".join(watched[:20]))
