...
- **写入前必读对应模板**（research / practice / devlog 三选一）与 `references/frontmatter-tags.md`，严格套用其模板、清单与格式——不要凭印象写，这是防格式飘的关键。抽取时执行铁律二、三；过滤噪音时对照 `references/anti-patterns.md`（六类坏例与改写）；排版按 frontmatter-tags 的 layout-rule 克制用 markdown（每元素一职责、段落优先、callout 克制）。
