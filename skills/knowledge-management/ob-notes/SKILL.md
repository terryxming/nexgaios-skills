...
- **写入前必读对应模板**（research / practice / devlog 三选一）与 `references/frontmatter-tags.md`，严格套用其模板、清单与格式——不要凭印象写，这是防格式飘的关键。抽取时执行铁律二、三；过滤噪音时对照 `references/anti-patterns.md`（坏例与改写）；排版按 frontmatter-tags 的 layout-rule 克制用 markdown（每元素一职责、段落优先、callout 克制）。
...
- `references/mode-a-research.md` / `mode-a-practice.md` / `mode-b-devlog.md` — **判定类型后读对应一个**：模板 + 该记 / 该丢清单 + 边界示例（research 另含 source-fidelity 原文保真与 mastery-lens 掌握视角；mode-b 另含与 CLAUDE.md/AGENTS.md 打通）。
