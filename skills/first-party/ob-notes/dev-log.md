...
| 2026-07-06 | **追问链答从 transcript 逐字扣（宿主 ADR-0013）**：新增 `transcript-extract` 规则项 + `scripts/extract_transcript.py`；结构层机械滤噪、语义层滤操作旁白（选 B）；「问保真答重写」在追问链下让位逐字扣、综合呈现（主题网/解法卡）仍适用 | 「答重写」本身是又一次 LLM 生成、受概率性支配（同段答重写两次都不同、都对不上原话）；长会话上下文被压缩后 agent 记忆已是压缩版、连逐字原料都没有 → 唯一 ground truth 是会话 jsonl。dogfood（当前会话 210 行）：9 轮问答逐字保真、噪音全滤、多段答归并；暴露操作旁白需语义层处理 | 保留纯 agent 重写（否：概率漂移 + 压缩失真）；逐字扣靠 agent 工作记忆而非 jsonl（否：记忆已被压缩污染）；操作旁白全留（否：过场噪音，选 B 滤） |

## 当前状态 / 下一步（覆盖更新）

- 现状：**v1.0.0 候选（架构翻转已落地，见宿主 ADR-0011）**：以问答为基底，workflow = 先信号/噪音分离（`distill.md` 引擎：三限定判据 + 问保真答重写四镣铐 + 操作留结果不留过程 + source-fidelity）再按复用目标定呈现侧重（`presentation.md`：追问链/解法卡有骨架、主题网去骨架、mastery-lens 挂主题网）；三 mode-a 文件已删、SKILL workflow 翻转、受控词表 26→25、depmap 全绿、5 处活跃死引用已修。当前改动在分支 `codex/ob-notes-obsidian-only`，待评测与发布门禁完成后才能合入 main 并打 tag。**2026-07-06 追问链改逐字扣（ADR-0013）已落地**：新增规则项 `transcript-extract` + `scripts/extract_transcript.py`（dogfood 当前会话 210 行 / 9 轮提取通过），distill/presentation/SKILL 改、maintenance 登记、depmap 27 项全绿；待补 evals 转正与跨文件回溯 / Codex 侧实机验证。
- 下一步：
  1. **重写 evals 执行用例**：覆盖信号/噪音分离、（综合呈现）答重写、**（追问链）transcript-extract 逐字扣 + 操作旁白滤除**、操作过程滤除、无问存档划出、主题网去骨架；**转正 pending 用例 `2026-07-05-answer-rewrite-vs-verbatim`**（触发用例基本不变）。
  2. **跑宿主 lint 全绿**（结构门禁）。
...
- 2026-07-04：**失败回填通道落地（宿主 ADR-0012，feedback-loop）**。动机：真实使用必有失败案例，需自动化回流给迭代；边界先划清——发现失败（语义判断）与用例审批（F 人在环）原理上不可自动化，自动化目标=捕获零摩擦、遗失零容忍。四件构件：①捕获——用户说一句"回填用例"，agent 按 `references/feedback.md` 把现场（触发语/坏笔记摘录原样、用户原话逐字保真、疑似规则项、断言草稿）写成 `evals/pending/` 一个 JSON；config 可选键 `dev_repo` 门控（普通用户惰性，skill 源不写机器路径、过桶二·1）；②提醒——pending 只写文件不 commit，留 dirty 给 C1 巡检看见，零新增提醒机制；③转换——agent 起草 eval、用户过目（F）、入 evals.json 删暂存；④发布闸——宿主 `lint.py --release` pending 非空即红（平时 CI 不查，防堵捕获通道）。与已删 monitoring 的区别=事件驱动、仅维护者，非每次沉淀无条件记录。词表 25→26，SKILL 维护节挂引导。否决：每次沉淀记 jsonl（即被删的 monitoring）；独立 feedback skill（新发布单元过重）；暂存放 ~/.config（CI 与 C1 都看不见，会遗失）。通道全链路未经真实失败案例走通，试行待验证。
