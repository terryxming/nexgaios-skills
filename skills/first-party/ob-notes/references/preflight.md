...
3. 都没有 → **停下来问用户**知识库根路径，绝不猜测、绝不用任何默认值写盘（与铁律一·先校验，绝不写到不确定的地方一致）。
...
| 用途 | 落点 |
|---|---|
| 对话价值沉淀 | `{kb_root}/{inbox_subpath}/`（`inbox_subpath` 默认 `00 - raw/00 - inbox`） |

说明：

- **入口子路径可配**：落点子路径默认 `00 - raw/00 - inbox`（贴合常见 Obsidian「inbox」约定）。若你的库结构不同，可在 `config.json`（与 `kb_root` 同一文件，见第 1 节）里加可选键 `inbox_subpath` 覆盖；**未配置则用默认**，行为与旧版完全一致。示例：`{ "kb_root": "<盘符>:\\my-kbase", "inbox_subpath": "10 - Inbox" }`。
...
1. **解析落点**：先解析 `{kb_root}`（第 1 节），再按 landing-rule（第 2 节）拼出确切目标目录（默认 `{kb_root}/00 - raw/00 - inbox/`，或 `inbox_subpath` 覆盖后的子路径）。未配置则停下来问。
2. **存在性检查**：目标目录是否存在？
   - 存在 → 继续。
   - 不存在 → **停下来问用户**："目标目录 `<路径>` 未找到，是路径变了，还是要我在此创建？" 由用户拍板，**绝不静默新建**、绝不退而写到当前目录（铁律一·先校验，绝不写到不确定的地方）。
...
原则与铁律一·先校验，绝不写到不确定的地方一致：路径只要有不确定，就停下来问，不赌。
