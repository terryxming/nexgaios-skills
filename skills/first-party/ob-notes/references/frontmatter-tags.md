...
- **标题三处关系**：文件名 = 显示标题（见 naming-rule），靠 Obsidian"显示行内标题"显示为大标题；正文**不再写 H1**，避免与行内标题重复成双标题；frontmatter 的 `title` 作为标题的显式记录（供搜索 / Dataview / 导出），与文件名保持一致。例外：dev-log 文件名固定，标题由 H1 承载。
...
- **双链** `[[笔记名]]`：用于缝合相关笔记。交叉场景（见 SKILL.md mode-decision）下，主笔记用双链回指另一侧产物。不滥用——只在真有关联时建链，避免制造噪音链。
  ...
  - **目标尚未建时显式标注，不留隐形死链**：若确实想预留指向一篇"该建但还没建"的笔记，在链后加 `(待建)`，如 `[[Loop Engineering 是什么]] (待建)`——把它变成一个明确的待办钩子，复盘时据此补建，而非一条看不见的断链。
...
- 开发日志：**例外**——固定名 `dev-log.md`（它跟项目走、文件名不表标题，标题由 H1 承载；落点见 preflight.md landing-rule）。
- **只规避文件系统非法字符** `/ \ : * ? " < > |`；中文、空格、括号都可保留。标题要改就走 Obsidian 内置改名（会自动修双链）。

> 取舍：旧版用英文 slug 图的是跨工具 / 链接稳定，但本 skill 落点固定是 Obsidian 库、且改名会自动改链，这层收益基本不成立，反而天天要做心智映射，故改为"文件名 = 标题"。**唯一代价**：标题（=文件名）改动会牵动 git 历史与监控层 `target` 路径，所以标题尽量一次起好、要改走 Obsidian 改名。若日后本库要跨工具 / 对外发布，再回头评估 slug。
...
格式服务阅读，不服务炫技。**核心原则：每个 markdown 元素只承担一个职责，用对、用少**——堆砌语法 ≠ 美观（堆砌的坏相见 anti-patterns 第 5 条"格式炫技"，本节是它的正面规约）。callout / 双链 / 代码块的用途细节见本文件第 5 节 linking-convention，不在此重复。
