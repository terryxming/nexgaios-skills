...
本文件是所有笔记**格式细节**的唯一真相源。distill 引擎、三种呈现侧重（presentation）和铁律三、四都引用这里，不在别处重复定义。**写任何笔记前必读本文件。**
...
- **标题三处关系**：文件名 = 显示标题（见 naming-rule），靠 Obsidian"显示行内标题"显示为大标题；正文**不再写 H1**，避免与行内标题重复成双标题；frontmatter 的 `title` 作为标题的显式记录（供搜索 / Dataview / 导出），与文件名保持一致。
...
格式服务阅读，不服务炫技。**核心原则：每个 markdown 元素只承担一个职责，用对、用少**。堆砌语法不等于美观（堆砌的坏相见 anti-patterns 第 5 条"格式炫技"，本节是它的正面规约）。callout / 双链 / 代码块的用途细节见本文件第 5 节 linking-convention，不在此重复。
