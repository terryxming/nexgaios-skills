...
- `trigger_query` 与 `user_complaint` 逐字保真、不许改写
...
`python tools/lint.py --release` 检查 `evals/pending/` 必须为空——有未回填的失败案例不得发布（发布门禁前置，ADR-0012）。平时 CI 不查 pending，否则捕获文件一 push 就红、堵死捕获通道本身。
