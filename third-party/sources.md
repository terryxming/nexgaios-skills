...
> `third-party/` 存放从 GitHub 等处取回的第三方 skill **原样副本**，用于研究学习与本机安装（`python tools/install.py <name>`，同名冲突用 `--from third-party`）；不参与本仓任何门禁与发布——marketplace 门禁机械保证其进不了分发渠道（ADR-0010）。要改造，走 fork：复制到 `skills/<name>/` 后按仓库规则迭代（纪律 H）。
>
