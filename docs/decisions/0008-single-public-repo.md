...
- **存量违例(需尽快闭环)**:ob-notes v0.8.0 已在 main 但未打 v0.8.0 tag(基线仅 v0.7.0)——早于方案甲生效,属历史遗留;应尽快走完 G 发布(评测 k=1 已过,G⑤ 人在环后打 tag),此后 skill 改动一律走分支。
- 本地目录名仍为 `nexgaios-skills-dev`,与远端名不一致——**改名留给用户在会话外操作**(会话中改动 cwd 会破坏工具链;git 不受目录名影响)。
