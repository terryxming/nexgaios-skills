...
1. **skills/ 内二分**:`skills/first-party/`(自主开发,发布单元)与 `skills/third-party/`(第三方**原样副本**,逐字节不改,保 diff 上游的能力)。first-party 是 third-party 的标准反义配对(A3 通用词);`skills/` 一级只允许这两个分类目录,lint 布局检查机械防呆(skill 落错层即红)。
