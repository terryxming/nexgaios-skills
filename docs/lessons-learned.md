...
| 2026-07-04 | 流水线文件提交落错分支 | 上一批收尾停在 feature 分支未切回，下一批开工未复核当前分支就动手——纪律/README 改动被提交到 candidate 分支而非 main（后 cherry-pick 修复，内容一致故发布合并无冲突） | 任何 commit 前先看 `git branch --show-current`；跨批次收尾时显式切回 main | **复发 → 毕业**（见 2026-07-06 行）：CLAUDE/AGENTS 纪律 C5 |
...
| 2026-07-06 | 多 agent 共享 worktree 切 HEAD（『落错分支』复发升级） | 2026-07-04 同类坑复发并加重：多 agent 共享单一工作区 `D:\nexgaios-skills`，HEAD 是全局的、被并行 Codex 会话切回 codex 分支；本会话 commit learn-everything 落到 codex 而非独立分支，随后清理时工作区文件"消失"（实为切分支所致），返工用 `git worktree` 隔离修复 | 一 skill 一分支一 worktree（物理隔离、独立 HEAD、文件恒可见、并行零踩踏）；commit/reset/add 前先 `git branch --show-current` | **毕业**：CLAUDE.md/AGENTS.md 纪律 **C5**；memory `shared-worktree-verify-branch` |
