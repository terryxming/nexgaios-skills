...
| 2026-07-04 | 流水线文件提交落错分支 | 上一批收尾停在 feature 分支未切回，下一批开工未复核当前分支就动手——纪律/README 改动被提交到 candidate 分支而非 main（后 cherry-pick 修复，内容一致故发布合并无冲突） | 任何 commit 前先看 `git branch --show-current`；跨批次收尾时显式切回 main | 复发（见 2026-07-06 行）；暂留为知识（切分支/commit 前核对当前分支） |
...
| 2026-07-06 | 单一工作区多线程切 HEAD（『落错分支』复发） | 2026-07-04 同类坑复发：两个 Claude 线程共享单一工作区 `D:\nexgaios-skills`，HEAD 全局、被并行线程（开发 ob-notes 的那个）切回 codex 分支；本会话 commit learn-everything 落到 codex 而非其分支，随后清理时工作区文件"消失"（实为切分支所致），返工修复 | 保持标准单仓：各 skill 各自分支、共享一个工作区，切分支/commit 前核对 `git branch --show-current`、多线程错开开发不同 skill（曾试行"一 skill 一 worktree"C5 后判过度复杂、已撤） | 暂留为知识；memory `shared-worktree-verify-branch` |
